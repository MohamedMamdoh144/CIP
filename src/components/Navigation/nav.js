...
function NavBtn({width,height,text,setshow,setpage,id,hover,sethover,setclose}){
...
function NavElements({show,setshow,setpage,setclose}){
    const elms=['Scene 1','Scene 2','Management',
    ...
    function render(){
        return(
            elms.map((el,i)=><NavBtn width={(width)+"px"} height={"100px"} text={el} setshow={setshow} setpage={setpage} id={i} hover={hovered} sethover={setHovered} setclose={setclose} />)
        )
    }
    return(
...
function Navigation({show,setshow,setpage,setclose}){
    const STATE_MACHINE_NAME="State Machine 1";
    const { rive, RiveComponent } = useRive({
        src: 'back.riv',
        autoplay: true,
    ...
    return(
        <div className='nav-container' ref={ref}>
            ...
            <NavElements show={show} setshow={setshow} setpage={setpage} setclose={setclose} />
